...
{
    mesh result;
    result.position.resize(4);
    result.connectivity.resize(4);

    // Points de la pyramide
    float angle1 = pi / 2.0f;
    float angle2 = 7 * pi / 6;
    float angle3 = -pi / 6;
    result.position[0] = get_point_on_sphere(vec3(1.0f, 1.0f, 1.0f));
    result.position[1] = get_point_on_sphere(vec3(1.0f, -1.0f, -1.0f));
    result.position[2] = get_point_on_sphere(vec3(-1.0f, 1.0f, -1.0f));
    result.position[3] = get_point_on_sphere(vec3(-1.0f, -1.0f, 1.0f));
    // Triangles de la pyramide
    result.connectivity[0] = uint3(0, 1, 2);
    result.connectivity[1] = uint3(0, 1, 3);
    result.connectivity[2] = uint3(1, 2, 3);
    result.connectivity[3] = uint3(2, 0, 3);
...
vec3 perturbation() {
    const float norm = pow(sphere_radius, 2) / pow(number_of_steps, 2);
