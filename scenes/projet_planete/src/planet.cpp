...
planet::planet(user_parameters* user) : scene_visual(user)
{
    // Initialisation des shaders
    planet_shader = opengl_create_shader_program(open_shader("planet_vert"), open_shader("planet_frag"));
    normal_shader = opengl_create_shader_program(open_shader("normal_vert"), open_shader("normal_frag"));
...
void planet::update_visual(GLFWwindow* window, double mouse_x_pos, double mouse_y_pos)
{

    vec2 const  p1 = glfw_get_mouse_cursor(window, mouse_x_pos, mouse_y_pos);
    vec2 const& p0 = user_reference->mouse_prev;
    glfw_state state = glfw_current_state(window);

    if (!user_reference->cursor_on_gui && !state.key_shift) {
    ...
    if (state.key_shift)
    {
        ...
        if (state.mouse_click_left && picking.active)// Action lors du clic
...
void planet::set_islands() 
{
    ...
    for (int i = 0; i < number_of_islands; i++) {
        ...
        island_visual = mesh_drawable(island_mesh, normal_shader);
...
void planet::set_skybox() {
    skybox.init_skybox(vec3(0, 0, 0), 10, "space", normal_shader);
}
