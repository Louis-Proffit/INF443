...
float coeff_grass(vec3 position);
...
void main()
{
	...
	vec4 color_image_texture = (1 - coeff_grass(fragment.position)) * texture(image_texture, uv_image) + coeff_grass(fragment.position) * texture(image_texture_grass, uv_image);
