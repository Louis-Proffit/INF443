...
namespace vcl
{
	struct mesh_float_drawable
	{
		...
		void clear();
		mesh_float_drawable& update_position(buffer<vec3> const& new_position);
		mesh_float_drawable& update_normal(buffer<vec3> const& new_normal);
	...
	void draw(mesh_float_drawable const& drawable, SCENE const& scene);

	template <typename SCENE>
	void draw_wireframe(mesh_float_drawable const& drawable, SCENE const& scene, vec3 const& color={0,0,1});
}
...
namespace vcl
{
	...
	template <typename SCENE>
	void draw_wireframe(mesh_float_drawable const& drawable, SCENE const& scene, vec3 const& color)
	{
		mesh_float_drawable wireframe = drawable;
