#include "mesh_float_drawable.hpp"

#include "vcl/base/base.hpp"

namespace vcl
{
	GLuint mesh_float_drawable::default_shader = 0;
	GLuint mesh_float_drawable::default_texture = 0;


	mesh_float_drawable::mesh_float_drawable()
		:vbo(), vao(0), shader(0), texture(0), transform(), shading()
	{}

	mesh_float_drawable::mesh_float_drawable(mesh const& data_to_send, buffer<float> float_buffer, buffer<vec3> vec_buffer, GLuint shader_arg, GLuint texture_arg, GLuint draw_type)
		:vbo(), vao(0), shader(shader_arg), texture(texture_arg), transform(), shading()
	{
		...
		opengl_create_gl_buffer_data(GL_ARRAY_BUFFER, vbo["float"], float_buffer, draw_type);
		opengl_create_gl_buffer_data(GL_ARRAY_BUFFER, vbo["vec"], vec_buffer, draw_type);
		...
		opengl_set_vertex_attribute(vbo["float"],    4, 1, GL_FLOAT);
		opengl_set_vertex_attribute(vbo["vec"],		 5, 3, GL_FLOAT);
		glBindVertexArray(0);      opengl_check
	}


	mesh_float_drawable& mesh_float_drawable::update_position(buffer<vec3> const& new_position)
	{
	...
	mesh_float_drawable& mesh_float_drawable::update_normal(buffer<vec3> const& new_normals)
	{
	...
	mesh_float_drawable& mesh_float_drawable::update_color(buffer<vec3> const& new_color)
	{
	...
	mesh_float_drawable& mesh_float_drawable::update_uv(buffer<vec2> const& new_uv)
	{
	...
	mesh_float_drawable& mesh_float_drawable::update_float(buffer<float> const& new_float)
	{
		glBindBuffer(GL_ARRAY_BUFFER,vbo["float"]); opengl_check;
		glBufferSubData(GL_ARRAY_BUFFER,0,size_in_memory(new_float),ptr(new_float));  opengl_check;
		return *this;
	}

	mesh_float_drawable& mesh_float_drawable::update_vec(buffer<vec3> const& new_vec)
	{
		glBindBuffer(GL_ARRAY_BUFFER, vbo["vec"]); opengl_check;
		glBufferSubData(GL_ARRAY_BUFFER, 0, size_in_memory(new_vec), ptr(new_vec));  opengl_check;
		return *this;
	}

	void mesh_float_drawable::clear()
	{
		for(auto& buffer : vbo)
