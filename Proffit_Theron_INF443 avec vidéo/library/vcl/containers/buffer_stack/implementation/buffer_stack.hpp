...
namespace vcl
{
    ...
    template <typename T, size_t N>
    std::ostream &operator<<(std::ostream &s, buffer_stack<T, N> const &v);

    /** Direct compiled-checked access to data */
    template <size_t idx, typename T, size_t N>
    T const &get(buffer_stack<T, N> const &data);
    template <size_t idx, typename T, size_t N>
    T &get(buffer_stack<T, N> &data);

    /** Convert all elements of the buffer to a string.
     * \param buffer: the input buffer
     * \param separator: the separator between each element */
    template <typename T, size_t N>
    std::string str(buffer_stack<T, N> const &v, std::string const &separator = " ", std::string const &begin = "", std::string const &end = "");
    template <typename T, size_t N>
    std::string type_str(buffer_stack<T, N> const &);
    template <typename T, size_t N>
    size_t size_in_memory(buffer_stack<T, N> const &v);
    template <typename T, size_t N>
    auto const *ptr(buffer_stack<T, N> const &v);
    ...
    bool is_equal(buffer_stack<T1, N1> const &a, buffer_stack<T2, N2> const &b);
    template <typename T, size_t N>
    bool is_equal(buffer_stack<T, N> const &a, buffer_stack<T, N> const &b);

    /** Compute average value of all elements of the buffer. \relates buffer_stack \ingroup container */
    template <typename T, size_t N>
    T average(buffer_stack<T, N> const &a);

    /** Math operators
     * Common mathematical operations between buffers, and scalar or element values. */
    template <typename T, size_t N>
    buffer_stack<T, N> operator-(buffer_stack<T, N> const &a);

    template <typename T, size_t N>
    buffer_stack<T, N> &operator+=(buffer_stack<T, N> &a, buffer_stack<T, N> const &b);
    template <typename T, size_t N>
    buffer_stack<T, N> &operator+=(buffer_stack<T, N> &a, T const &b);
    template <typename T, size_t N>
    buffer_stack<T, N> operator+(buffer_stack<T, N> const &a, buffer_stack<T, N> const &b);
    template <typename T, size_t N>
    buffer_stack<T, N> operator+(buffer_stack<T, N> const &a, T const &b);
    template <typename T, size_t N>
    buffer_stack<T, N> operator+(T const &a, buffer_stack<T, N> const &b);
    ...
    buffer_stack<T, N> &operator*=(buffer_stack<T, N> &a, buffer_stack<T, N> const &b);
    template <typename T, size_t N>
    buffer_stack<T, N> &operator*=(buffer_stack<T, N> &a, float b);
    template <typename T, size_t N>
    buffer_stack<T, N> operator*(buffer_stack<T, N> const &a, buffer_stack<T, N> const &b);
    template <typename T, size_t N>
    buffer_stack<T, N> operator*(buffer_stack<T, N> const &a, float b);
    template <typename T, size_t N>
    buffer_stack<T, N> operator*(float a, buffer_stack<T, N> const &b);
    ...
    //  Condition: ||v||>0
    template <typename T, size_t N>
    buffer_stack<T, N> normalize(buffer_stack<T, N> const &v);
...
namespace vcl
{
    ...
    template <typename T, size_t N>
    std::size_t buffer_stack<T, N>::size() const
    {
        return N;
    }

    template <typename T, size_t N>
    buffer_stack<T, N> &buffer_stack<T, N>::fill(T const &value)
    {
        for (size_t k = 0; k < N; ++k)
            data[k] = value;
        return *this;
    ...
    template <typename T, size_t N>
    T const &buffer_stack<T, N>::operator[](int index) const
    {
        check_index_bounds(index, *this);
        return data[index];
    }
    template <typename T, size_t N>
    T &buffer_stack<T, N>::operator[](int index)
    {
        check_index_bounds(index, *this);
        return data[index];
    }
    ...
    template <typename T, size_t N>
    T const &buffer_stack<T, N>::operator[](std::size_t const index) const
    {
        check_index_bounds(index, *this);
        return data[index];
    }
    template <typename T, size_t N>
    T &buffer_stack<T, N>::operator[](std::size_t const index)
    {
        check_index_bounds(index, *this);
        return data[index];
    }
    template <typename T, size_t N>
    T const &buffer_stack<T, N>::operator()(std::size_t index) const
    {
        check_index_bounds(index, *this);
        return data[index];
    }
    ...
    template <typename T, size_t N>
    T &buffer_stack<T, N>::operator()(std::size_t index)
    {
        check_index_bounds(index, *this);
        return data[index];
    }
    ...
    template <typename T, size_t N>
    T const &buffer_stack<T, N>::at(std::size_t index) const
    {
        return data.at(index);
    }

    template <typename T, size_t N>
    T &buffer_stack<T, N>::at(std::size_t index)
    {
        return data.at(index);
    }

    template <typename T, size_t N>
    typename std::array<T, N>::iterator buffer_stack<T, N>::begin()
    {
        return data.begin();
    }
    template <typename T, size_t N>
    typename std::array<T, N>::iterator buffer_stack<T, N>::end()
    {
        return data.end();
    }
    template <typename T, size_t N>
    typename std::array<T, N>::const_iterator buffer_stack<T, N>::begin() const
    {
        return data.begin();
    }
    template <typename T, size_t N>
    typename std::array<T, N>::const_iterator buffer_stack<T, N>::end() const
    {
        return data.end();
    }
    template <typename T, size_t N>
    typename std::array<T, N>::const_iterator buffer_stack<T, N>::cbegin() const
    {
        return data.cbegin();
    }
    template <typename T, size_t N>
    typename std::array<T, N>::const_iterator buffer_stack<T, N>::cend() const
    {
        return data.cend();
    }
    ...
    template <typename T, size_t N>
    T dot(buffer_stack<T, N> const &a, buffer_stack<T, N> const &b)
    {
        T s = {};
        for (size_t k = 0; k < N; ++k)
            s += a[k] * b[k];
    ...
    template <typename T, size_t N>
    T norm(buffer_stack<T, N> const &a)
    {
        using std::sqrt;
        return sqrt(dot(a, a));
    }
