...
float coeff_snow(vec3 position);
...
void main()
{
	...
	vec4 color_image_texture = (1 - coeff_snow(fragment.position)) * texture(image_texture, uv_image) + coeff_snow(fragment.position) * texture(image_texture_snow, uv_image);
