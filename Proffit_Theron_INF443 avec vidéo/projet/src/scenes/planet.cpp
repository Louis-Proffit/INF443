...
{
    GLuint planet_shader = get_shader(shader_type::PLANET);
...
void planet::set_planet()
{
    ...
    planet_visual = mesh_float_drawable(planet_mesh, noise, parrallels, get_shader(shader_type::PLANET));
}
...
void planet::set_islands()
{
    ...
    GLuint texture_id = scene_visual::get_texture(texture_type::LOWPOLY);
    ...
    for (int i = 0; i < number_of_islands; i++)
    {
        ...
        island_visual = mesh_drawable(island_mesh, get_shader(shader_type::NORMAL));
...
{
    _skybox.init_skybox(vec3(0, 0, 0), 10, skybox_type::SPACE, get_shader(shader_type::NORMAL));
}
...
void planet::set_sun()
{
    ...
    sun_visual = mesh_drawable(sun, get_shader(shader_type::SUN));
